...
func TestJSONMarshalUnmarshalCredential(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assertJSONMarshalEqual(t, &tc.v, &decoded)
		})
	}
}
...
func BenchmarkJSONMarshalCredentials(b *testing.B) {
	v := Credentials{newPopulatedCredential(), newPopulatedCredential(), newPopulatedCredential()}
...
// assertJSONMarshalEqual asserts that two values produce equivalent JSON. It is preferred over
...
func assertJSONMarshalEqual(t *testing.T, original, decoded any) {
	t.Helper()
